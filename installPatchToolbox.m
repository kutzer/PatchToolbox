...
%   locations:
%                             Source: Destination
%           PatchToolboxFunctions: matlabroot\toolbox\patch
%
%   INSTALLPATCHTOOLBOX(true) installs Patch Toolbox regardless of
%   whether a copy of the Patch toolbox exists in the MATLAB root.
%
%   INSTALLPATCHTOOLBOX(false) installs Patch Toolbox only if no copy 
%   of the Patch toolbox exists in the MATLAB root.
%
...
if isToolbox == 7
    ...
    switch choice
        case 'Yes'
            ...
            if isRemoved
                fprintf('Previous version of Patch Toolbox removed successfully.\n');
            else
                fprintf('Failed to remove old Patch Toolbox folder:\n\t"%s"\n',toolboxRoot);
                fprintf(adminSolution);
                error(msgID,msg);
            end
        case 'No'
            fprintf('Patch Toolbox currently exists, installation cancelled.\n');
            return
        case 'Cancel'
...
if isDir
    fprintf('Patch toolbox folder created successfully:\n\t"%s"\n',toolboxRoot);
else
...
toolboxContent = 'PatchToolboxFunctions';
...
wb = waitbar(0,'Copying Patch Toolbox toolbox contents...');
n = numel(files);
fprintf('Copying Patch Toolbox contents:\n');
for i = 1:n
